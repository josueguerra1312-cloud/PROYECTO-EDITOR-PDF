...
AUTH_VERSION = "v9-required-login-2026-07-15"
...
def require_login():
    # El token de version invalida sesiones autenticadas por despliegues anteriores.
    authenticated = (
        st.session_state.get("authenticated") is True
        and st.session_state.get("auth_version") == AUTH_VERSION
    )
    if authenticated:
        with st.sidebar:
            st.write(f"Usuario: **{st.session_state['username']}**")
            if st.button("Cerrar sesion", use_container_width=True, key="logout_v9"):
                for key in list(st.session_state.keys()):
                    del st.session_state[key]
                st.rerun()
        return

    # Borra cualquier sesion antigua para que nunca omita el formulario.
    ...
    with st.form("login_form_v9"):
        username = st.text_input("Usuario", key="login_username_v9")
        password = st.text_input("Contrasena", type="password", key="login_password_v9")
